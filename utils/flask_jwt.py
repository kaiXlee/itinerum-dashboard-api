...
"""
    ...
    Forked Flask-JWT module
    https://github.com/mattupstate/flask-jwt

    Modified by Kyle Fitzsimmons, 2016
"""
...
from flask import current_app, request, jsonify, _request_ctx_stack, make_response
from werkzeug.local import LocalProxy

from utils.flask_jwt_addons import survey_instance_id, survey_instance_name, user_role_level
...
def _default_jwt_payload_handler(identity):
    ...
    survey_id = survey_instance_id(identity)
    identity = getattr(identity, 'id') or identity['id']
    return {'exp': exp, 'iat': iat, 'nbf': nbf, 'identity': identity, 'survey_id': survey_id}
...
def _default_auth_response_handler(access_token, identity):
    payload = {
        ...
        'surveyName': survey_instance_name(identity),
        'userLevel': user_role_level(identity)
    }
...
def _jwt_required(realm):
    ...
    if token is None:
        return JWTError('Authorization Required', 'Request does not contain an access token',
...
def jwt_required(realm=None):
    ...
    def wrapper(fn):
        ...
        def decorator(*args, **kwargs):
            error = _jwt_required(realm or current_app.config['JWT_DEFAULT_REALM'])
            if not error:
                ...
                return make_response(error_response, error.status_code)
        return decorator
    return wrapper
...
def jwt_refresh(token):
    """Generates a new payload for an active authenticating tokens"""
    secret = current_app.config['JWT_SECRET_KEY']
    algorithm = current_app.config['JWT_ALGORITHM']
    leeway = current_app.config['JWT_LEEWAY']
    verify_claims = current_app.config['JWT_VERIFY_CLAIMS']
    ...
    try:
        old_payload = jwt.decode(token, secret,
            options=options,
        ...
        return make_response(jsonify({
            'error': 'Session token is expired, please login'
        }), 401)
    ...
    new_payload = {
        ...
        'exp': iat + current_app.config.get('JWT_EXPIRATION_DELTA'),
        'nbf': iat + current_app.config.get('JWT_NOT_BEFORE_DELTA'),
        'identity': old_payload['identity'],
        'survey_id': old_payload['survey_id']
    }

    user_level = 2
    identity = _jwt.identity_callback(old_payload)
    if identity.has_role('researcher'):
        user_level = 1
    if identity.has_role('admin'):
        user_level = 0

    token = jwt.encode(new_payload, secret, algorithm=algorithm, headers=None).decode('utf-8')
    payload = {
        'accessToken': token,
